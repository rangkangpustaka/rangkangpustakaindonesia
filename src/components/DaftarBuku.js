...
export default function DaftarBuku({ isAdmin }) {
  ...
  const handleExportExcel = () => {
    ...
    const csvData = dataEkspor.map((item, index) => [
      ...
      `"${item.sumber || "-"}"`, item.stok, `"${item.sampul || ""}"`
    ]);
                    ...
                    <div className="flex-1 flex flex-col justify-between min-h-[112px]">
                      <div>
                        <div className="flex gap-1.5 mb-1 flex-wrap">
                          <span className="text-[10px] font-extrabold bg-gray-100 text-gray-700 px-2 py-0.5 rounded-md border border-gray-200">{item.kategori}</span>
